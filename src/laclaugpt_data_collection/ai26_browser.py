"""AI26 Firefox runtime using the shared localhost collector and distributed storage.

Firefox talks only to a loopback capture server. Canonical JSONL is retained
locally as a recovery/debug fallback and mirrored continuously to the configured
MongoDB/Redis/S3 (CSC Allas) backends. Media downloading is intentionally kept
out of the interactive browser request path and is run separately, normally by
cron.
"""
...
def _ensure_ai26_study(study_config: str | Path) -> str:
    cfg = load_config(study_config)
    study = str(cfg.study)
    token = study.casefold().replace("_", "-").replace(" ", "-")
    if token != "ai26" and not token.startswith("ai26-"):
        raise ValueError(f"AI26 runtime refuses non-AI26 study config: study={study!r}")
    return study
...
class JsonlDistributedMirror:
    """Mirror newly appended canonical JSONL rows to the remote AI26 stack."""
    ...
    def __init__(
        ...
        *,
        ...
        self.state_path = self.data_root / ".ai26-distributed-offsets.json"
            ...
            with path.open("r", encoding="utf-8") as handle:
                ...
                while True:
                    ...
                    if collection_id != PROJECT_ID:
                        raise ValueError(
                            f"AI26 mirror refuses collection_id={collection_id!r} in {path.name}"
                        )
...
def run_backend(
    *,
    ...
    if host not in _LOOPBACK:
        raise ValueError("AI26 browser backend must bind to localhost/loopback")
    study = _ensure_ai26_study(study_config)
...
def run_media(
    *,
    ...
    _ensure_ai26_study(study_config)
    settings = _ai26_settings()
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m laclaugpt_data_collection.ai26_browser")
    sub = parser.add_subparsers(dest="command", required=True)

    backend = sub.add_parser("backend", help="run localhost Firefox capture with remote mirroring")
    backend.add_argument("--study-config", default=None)
    ...
    check = sub.add_parser("check", help="verify AI26 identity and remote control plane")
    check.add_argument("--study-config", default=None)

    media = sub.add_parser("media", help="download pending AI26 media to S3/Allas")
    media.add_argument("--study-config", default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    study_config = _study_config(args.study_config)
