"""Durable collection storage: raw captures, normalized JSONL, manifests, state.

Adapted from the public `collector/store.py` in
TomiToivio/LaclauGPT-Discourse-Analysis. Layout under one data root:
    ...
    <root>/normalized/<platform>.jsonl
        stable normalized records (deduplicated by platform+document_id)
    <root>/manifests/run-<UTC stamp>.json
        per-run manifest: config summary, per-account results, errors
    <root>/state.sqlite3
...
class CollectionStore:
    ...
    def upsert_post(self, record: dict, raw_ref: str) -> bool:
        """Insert a normalized record if new; returns True when inserted."""
